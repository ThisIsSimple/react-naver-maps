...
const kvoKeys = [...primitiveKeys, ...locationalKeys] as const;
...
export type Props = MarkerOptions &
...
function makeInitialOption(props: Props) {
  ...
  const prefixCleared = mapKeys(
    uncontrolledProps,
    (_, key) => uncontrolledKeyMap[key as keyof typeof uncontrolledKeyMap],
  );
...
export const Marker = forwardRef<naver.maps.Marker, Props>(
  function Marker(props, ref) {
    ...
    function getDirties() {
      ...
      return kvoKeys.reduce((acc, key) => {
        ...
        if (
          isLocationalKey(key) &&
          props[getUncontrolledKey(key)] !== undefined
        ) {
          return acc;
        }
